...
const CommandsPageContent = ({ userLoggedIn }: { userLoggedIn: boolean }) => {
  ...
  const [commandData, setCommandData] = useState<CommandsForm>({
    ...
    commandDescriptionForFunction: {
      ...
      error:
        "Invalid command function description. Please enter a description that is less than 1000 characters long (a-z)",
      valid: false,
      touched: false,
    },
  ...
  const onClearHandler = () => {
    setCommandData({
      ...
      commandDescriptionForFunction: {
        value: "",
        error:
          "Invalid command description. Please enter a valid description so the admins can create a command for you (1 - 1000 characters a-z)",
        valid: false,
        touched: false,
      },
      ...
      commandAuthorizedUser: {
        value: "",
        error:
          "Invalid authorization name. Please enter valid a valid authorization name that is less than 50 characters (1 - 50 characters a-z)",
        valid: false,
        touched: false,
      },
  ...
  const onSubmitHandler = async (
    ...
    const allFieldsValid: boolean = commandData.commandAuthorizedUsers.every(
      (user) => validateField("commandAuthorizedUser", user)
    );
    ...
    const formSubmissionData = {
      ...
      commandDescriptionForFunction:
        commandData.commandDescriptionForFunction.value,
      commandAuthorizedUsers: commandData.commandAuthorizedUsers,
    };

    try {
      const postCommandResponse = await postBotCommands(formSubmissionData);
    ...
    return (
      <main id="main" className="text-center">
        ...
        <aside id="bot_command_options_page_content">
          <h1 id="bot_command_options_page_title">Request bot command</h1>
        </aside>
          ...
          <Container>
            <Form>
                  ...
                  <FormGroup>
                    ...
                    <FormControl
                      ...
                      isInvalid={
                        ...
                        commandData.commandDescriptionForFunction.value.length >
                          0
                      }
                  ...
                  <Col xs={12} md={6} className="my-2" key={index}>
                    <FormGroup>
                      ...
                      <FormControl
                        ...
                        value={user}
                        onChange={(e) =>
                          onChangeAuthorizedUser(index, e.target.value)
                        }
                        isInvalid={
                          !!(
              ...
              <Row className="my-1 justify-content-center mt-5">
                <Col xs={12} md={6}>
                  <FormLabel className="command_authorized_user_label">
                    You can add a maximum of 8 authorized users. If you want
                    more, please specify so in the command description
                  </FormLabel>
                  <Button
